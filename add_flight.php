...
<aside class="right-side">                
    ...
    <section class="content-header">
        <h1>
            Adição de detalhes aéreos
            <small>Valores e Condições</small>
        </h1>
        <br/>
        <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> Home</a></li>
            <li class="active">Adicionar detalhes aéreos</li>
        </ol>
    </section>
    <!-- Main content -->                            
            ...
            <div class="tab-content">
                <div class="tab-pane active" id="tab_1">
                    ...
                    <div class="box-header">
                        <h3 class="box-title">Adicionando voo</h3>
                    </div><!-- /.box-header -->
                    <div class="box-body">
                        <form id="flight_form" method="POST" role="form" class="half-form" action="flight_information.php?page=add">						
                            <div class="form-group control-group">
								<label class="control-label" for="iata_arrival_at">Código IATA do aeroporto de chegada</label>
								<div class="controls">									 
									<input type="text" class="form-control" id="iata_arrival_at" data-inputmask="'mask': '(AAA)|(AA9)|(A9A)|(9AA)|(A99)|(9A9)|(99A)|(999)'" onfocus="active_autocomplete({id: this.id, target_field: 'airport_iata_code', search_field: 'airport_iata_code', table: 'digitulus_airport_list', validate: 'true'});"  name="iata_arrival_at" placeholder="Código IATA do aeroporto de chegada" onblur="load_airport_info();		create_flight_text(); validate({id: this.id, target_field: 'iata_arrival_at', search_field: 'iata_arrival_at', table: 'digitulus_flight', validate: 'true', page:'index.php?page=Editar-Voo&iata_arrival_at=', text:'Deseja alterar aeroporto de codigo iata:'})" required />
								</div>
                            </div>
							<div class="form-group control-group">
								<label class="control-label" for="arrival_at">Cidade de desembarque</label>
								<div class="controls">									 
									 <input type="text" class="form-control" id="arrival_at" name="arrival_at" placeholder="Cidade de desembarque" onfocus="this.form.departure_from.focus()" readonly required />
								</div>
                            </div>
							<div class="form-group control-group">
								<label class="control-label" for="country">País de destino</label>
								<div class="controls">									 
									<input type="text" class="form-control" id="country" name="country" placeholder="País de destino" onfocus="this.form.departure_from.focus()" readonly required />
								</div>
                            </div>
							<div class="form-group control-group">
								<label class="control-label" for="departure_from">Cidades de embarque</label>
								<div class="controls">	
									<input type="text" class="form-control" id="departure_from" name="departure_from" placeholder="Cidades de embarque" onfocus="active_autocomplete({id: this.id, target_field: 'departure_from', search_field: 'departure_from', table: 'digitulus_flight'});" onchange="create_flight_text()" required />
								</div>
                            </div>
							<div class="form-group control-group">
								<label class="control-label" for="flight_fare">Tarifa aérea (US$)</label>
								<div class="controls">
									<input type="text" class="form-control" id="flight_fare" name="flight_fare" placeholder="Tarifa aérea (US$)" onchange="create_flight_gross(); create_flight_net()" data-inputmask-alias="USD" required />
								</div>
                            </div>
							<div class="form-group control-group">
								<label class="control-label" for="airport_fee">Taxa de embarque (US$)</label>
								<div class="controls">
									<input type="text" class="form-control" id="airport_fee" name="airport_fee" placeholder="Taxa de embarque (US$)" onchange="create_flight_gross(); create_flight_net()" data-inputmask-alias="USD" required />
								</div>
                            </div>
							<div class="form-group control-group">
								<label class="control-label" for="flight_gross">Total bruto da parte aérea (US$)</label>
								<div class="controls">									 
									<input type="text" class="form-control" id="flight_gross" name="flight_gross" placeholder="Total bruto da parte aérea (US$)" data-inputmask-alias="USD" onfocus="this.form.flight_commission.focus()" readonly required />
								</div>
                            </div>
							<div class="form-group control-group">
								<label class="control-label" for="flight_commission">Comissão (%)</label>
								<div class="controls">									 
									 <input type="text" class="form-control" id="flight_commission" name="flight_commission" placeholder="Comissão (%)" onchange="create_total_flight_commission(); create_flight_net()" value="0" data-inputmask-alias="percentage" required />
								</div>
                            </div>
							<div class="form-group control-group">
								<label class="control-label" for="flight_incentive">Incentivo (%)</label>
								<div class="controls">									 
									<input type="text" class="form-control" id="flight_incentive" name="flight_incentive" placeholder="Incentivo (%)" onchange="create_total_flight_commission(); create_flight_net()" value="0" data-inputmask-alias="percentage" required />
								</div>
                            </div>
							<div class="form-group control-group">
								<label class="control-label" for="total_flight_commission">Total da comissão = Comissão + Incentivo (%)</label>
								<div class="controls">	
									<input type="text" class="form-control" id="total_flight_commission" name="total_flight_commission" placeholder="Total da comissão = Comissão + Incentivo (%)" data-inputmask-alias="percentage" value="0" onfocus="this.form.flight_dates.focus()" readonly required />
								</div>
                            </div>
							<div class="form-group control-group">
								<label class="control-label" for="flight_dates">Datas de validade da parte aérea</label>
								<div class="controls">
									<input type="text" class="form-control" id="flight_dates" name="flight_dates" placeholder="Datas de validade da parte aérea" onchange="create_flight_text()" onfocus="active_autocomplete({id: this.id, target_field: 'flight_dates', search_field: 'flight_dates', table: 'digitulus_flight'});" required />
								</div>
                            </div>
							<div class="form-group control-group">
								<label>Companhia aérea</label>
									<input type="text" class="form-control" id="air_company" name="air_company" placeholder="Companhia aérea" onchange="create_flight_text()" onfocus="active_autocomplete({id: this.id, target_field: 'airline_name', search_field: 'airline_name', table: 'digitulus_airline_list', validate: 'true'});" required />
                            </div>	
							<div class="form-group control-group">
								<label class="control-label" for="flight_net">Total líquido da parte aérea (US$)</label>
								<div class="controls">
									<input type="text" class="form-control" id="flight_net" name="flight_net" placeholder="Total líquido da parte aérea (US$)" data-inputmask-alias="USD" onfocus="this.form.submit_form.focus()" readonly required />
								</div>
                            </div>
							<div class="form-group control-group">
								<label class="control-label" for="flight_text">Texto das condições da parte aérea</label>
								<div class="controls">
									<input type="text" class="form-control" id="flight_text" name="flight_text" placeholder="Texto das condições da parte aérea" onfocus="this.form.submit_form.focus()" readonly required />
								</div>
                            </div>							
							<div class="form-group control-group">
								<div class="controls">
